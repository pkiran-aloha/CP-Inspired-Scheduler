...
export default function CommandPalette({ onClose, onNew, onHelp }) {
  ...
  const items = useMemo(() => {
    ...
    const secs = [['calendar', 'Cal', 'Calendar board'], ['clients', 'Cli', 'Clients roster'], ['staff', 'Stf', 'Staff directory'], ['billing', 'Bil', 'Billing & claims desk'], ['analytics', 'An', 'Analytics'], ['reports', 'Rep', 'Reports desk'], ['payers', 'Pay', 'Payer directory'], ['dashboard', 'Dash', 'Analytics dashboard']]
    secs.forEach(([id, , label], i) => out.push({ g: 'Jump to', k: String(i + 1), icon: id === 'calendar' ? 'cal' : id === 'clients' ? 'user' : id === 'staff' ? 'team' : id === 'billing' ? 'dollar' : id === 'analytics' ? 'spark' : id === 'payers' ? 'shield' : id === 'dashboard' ? 'dashboard' : 'clipboard', t: label, hint: 'section', run: go({ section: id }) }))
